...
import { formatALSGeneticHeadline, summarizeALSGeneticBackground } from '@/src/questionnaires/structured/alsGeneticBackground/labels';
...
export default function ALSGeneticBackgroundListScreen() {
    ...
    const { entries, deleteEntry } = useALSGeneticBackground();
    const [isEditing, setIsEditing] = useState(false);
    const isDE = i18n.language === 'de';
    const definition = getMetricDefinition('als_genetic_background', i18n.language);
    const canDelete = canWriteForActive || isDemo;

    if (!sharingLoaded || (isFiltering && !canSeeMetric('als_genetic_background'))) {
                            ...
                            {entries.map((entry, index) => (
                                <View key={entry.id} style={styles.listItemRow}>
                                    ...
                                    <View style={styles.listItemContent}>
                                        <List.Item
                                            title={formatALSGeneticHeadline(entry, i18n.language)}
