...
import { stageLabel, summarizeALSKingsStage } from '@/src/questionnaires/structured/alsKingsStage/labels';
import { fmtDateTime } from '@/src/lib/formatDate';
import { getMetricDefinition } from '@/src/metrics/definitions';

export default function ALSKingsStageListScreen() {
    ...
    return (
        <>
            {Platform.OS === 'android' ? (
                <Stack.Screen
                    options={{
                        headerTitle: definition?.name ?? (isDE ? "King's-Stage-Verlauf" : "King's Stage history"),
                ...
                <>
                    <Stack.Screen.Title>{definition?.name ?? (isDE ? "King's-Stage-Verlauf" : "King's Stage history")}</Stack.Screen.Title>
                            ...
                            {entries.map((entry, index) => (
                                <View key={entry.id} style={styles.listItemRow}>
                                    ...
                                    <View style={styles.listItemContent}>
                                        <List.Item
                                            title={stageLabel(entry.stage, i18n.language)}
