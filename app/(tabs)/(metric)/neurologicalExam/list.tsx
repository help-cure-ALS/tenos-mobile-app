...
import { summarizeNeurologicalExam } from '@/src/questionnaires/structured/neurologicalExam/labels';
import { fmtDateTime } from '@/src/lib/formatDate';
import { getMetricDefinition } from '@/src/metrics/definitions';

export default function NeurologicalExamListScreen() {
    ...
    const { entries, deleteEntry, isLoading } = useNeurologicalExams();
    const [isEditing, setIsEditing] = useState(false);
    const isDE = i18n.language === 'de';
    const definition = getMetricDefinition('als_neurological_exam', i18n.language);
    const canDelete = role === 'doctor' || isDemo;

    if (!sharingLoaded || (isFiltering && !canSeeMetric('als_neurological_exam'))) {
    ...
    return (
        <>
            {Platform.OS === 'android' ? (
                <Stack.Screen
                    options={{
                        headerTitle: definition?.name ?? (isDE ? 'Untersuchungsverlauf' : 'Exam history'),
                ...
                <>
                    <Stack.Screen.Title>{definition?.name ?? (isDE ? 'Untersuchungsverlauf' : 'Exam history')}</Stack.Screen.Title>
                            ...
                            {entries.map((entry, index) => (
                                <View key={entry.id} style={styles.listItemRow}>
                                    ...
                                    <View style={styles.listItemContent}>
                                        <List.Item
                                            title={entry.suggestedMotorNeuronCode ?? (definition?.name ?? (isDE ? 'Neurologische Untersuchung' : 'Neurological exam'))}
