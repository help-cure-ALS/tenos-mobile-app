...
export default function AddDoctorScreen() {
    ...
    const generateBundle = useCallback(async () => {
        ...
        try {
            ...
            // Patient responder: wait for the recipient's offer, authorize its device and
            // wrap the transport_key back. The patient seckey never leaves this device.
            ...
            if (e instanceof BundleCipherError) {
                setError(t('share.addDoctor.bundleKeyUnavailable'));
            } else {
    ...
    const handleCopy = useCallback(async () => {
        ...
        Alert.alert(t('share.addDoctor.copiedTitle'), t('share.addDoctor.copiedMessage'));
    }, [bundleText, t]);
    ...
    return (
        <View style={ [styles.container, { backgroundColor: colors.modalBackground }] }>
            {Platform.OS === 'android' ? (
                ...
                <Stack.Screen>
                    <Stack.Screen.Title>{t('share.addDoctor.title')}</Stack.Screen.Title>
                ...
                { error ? (
                    <View style={ styles.errorContainer }>
                        ...
                        <Text style={ [styles.errorTitle, { color: colors.text }] }>
                            { t('share.addDoctor.errorTitle') }
                        </Text>
                        <Text style={ [styles.errorText, { color: colors.textHint }] }>
                            { error }
                        </Text>
                        <Button
                            title={ t('share.addDoctor.retry') }
                    ...
                    <>
                            ...
                            <View style={ styles.qrContainer }>
                                <View style={ [styles.qrPlaceholder, { width: qrSize + 40, height: qrSize + 40, backgroundColor: colors.listItemBackground }] }>
                                    ...
                                    <Text style={ [styles.qrPlaceholderText, { color: colors.textHint }] }>
                                        { t('share.addDoctor.bundleTooLarge') }
                                    </Text>
                                </View>
                            </View>
                        ) : null }
                        ...
                        <List.Wrapper>
                            <Button
                                title={ t('share.addDoctor.copyCode') }
                                subtitle={ t('share.addDoctor.copyCodeSubtitle') }
                                onPress={ handleCopy }
                                disabled={ !bundleText }
                                variant="secondary"
                            />
