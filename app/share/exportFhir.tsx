...
import { exportBundleToShareSheet } from '@/src/lib/fhir-export/exportToShareSheet';
...
export default function ExportFhirScreen() {
    ...
    const handleExport = useCallback(async () => {
        ...
        try {
            ...
            await exportBundleToShareSheet(bundle);
        } catch (e: any) {
            Alert.alert(t('common.error'), e?.message ?? t('share.exportFhirError'));
        } finally {
            setIsExporting(false);
        }
    }, [activePatientId, unlock, metrics, metricToggles, categoryToggles, visibleCategories, allMetrics, allQuestionnaires, t]);
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.modalBackground }]}>
            {Platform.OS === 'android' ? (
                <Stack.Screen
                    options={{
                        headerTitle: t('share.exportFhir'),
                        headerRight: () => (
                            <HeaderButton
                                onPress={() => router.back()}
                                title={t('common.done')}
                ...
                <Stack.Screen>
                    <Stack.Screen.Title>{t('share.exportFhir')}</Stack.Screen.Title>
            ...
            >
                <ScreenHeader
                    subtitle={t('share.exportFhirDescription')}
                />
                ...
                <List.Section rounded>
                    <List.Item
                        title={t('share.exportFhirToggleAll')}
                        leftCmpSize={32}
                ...
                {metrics.length > 0 && (
                    <List.Section
                        title={t('share.exportFhirMetrics')}
                        rounded
                    >
                ...
                {visibleCategories.length > 0 && (
                    <List.Section
                        title={t('share.exportFhirCategories')}
                        rounded
                    >
            ...
            <View style={[styles.bottomBar, { paddingBottom: Math.max(insets.bottom, 16), backgroundColor: colors.modalBackground }]}>
                <Button
                    title={t('share.exportFhirButton')}
